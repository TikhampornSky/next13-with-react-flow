...
export default function UnorderedGroupNode({ id, data, isConnectable }: { id: string, data: { label: string }, isConnectable: boolean }) {
    ...
    return (
        <div style={{ backgroundColor: 'blue', display: 'flex', flexDirection: 'row', alignItems: 'center', width: `${nodeWidth * member.length}px`, height: `${nodeHeight}px` }}>
            ...
            <div style={{backgroundColor: 'red', display: 'flex', flexDirection: 'row'}}>
                {member.map((value) => {
                    return <div style={{ backgroundColor: '#ffabdc', marginRight: '5px', width: `${nodeWidth}px`, height: `${nodeHeight}px` }} key={value.id}> {value.name}</div>
                })}
            </div>
