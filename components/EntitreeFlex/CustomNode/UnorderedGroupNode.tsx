    ...
    return (
        <div style={{ backgroundColor: 'blue', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
            ...
            <div style={{backgroundColor: 'red', display: 'flex', flexDirection: 'row'}}>
                {member.map((value) => {
                    return <div style={{ backgroundColor: '#ffabdc', marginRight: '5px', width: `${defaultSettings.nodeWidth}px`, height: `${defaultSettings.nodeHeight}px` }} key={value.id}> {value.name}</div>
                })}
            </div>
